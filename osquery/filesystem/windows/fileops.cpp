...
const std::wstring kTrustedInstallerSID{L"S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464"};
...
static Status lowPrivWriteDenied(PACL acl) {
  ...
  auto trusted_installer_sid = static_cast<PSID>(trusted_installer_buffer.data());
  ...
  if (!CreateWellKnownSid(WinCreatorOwnerSid, nullptr, creator_owner_sid, &sid_buff_size)) {
    return Status(-1, "CreateWellKnownSid for CREATOR OWNER failed");
  }
  if (!ConvertStringSidToSid(kTrustedInstallerSID.c_str(), &trusted_installer_sid)) {
  ...
  for (unsigned long i = 0; i < acl->AceCount; i++) {
    ...
    if (entry->AceType == ACCESS_ALLOWED_ACE_TYPE) {
      ...
      // CREATOR OWNER is also allowed Full access, since the isOwnerRoot()
      // function will make sure that the owner is set to a correct value
