...
QueryData genSystemInfo(QueryContext& context) {
  ...
  const auto wmiSystemReq = WmiRequest::CreateWmiRequest("select * from Win32_ComputerSystem");
  const auto wmiSystemReqProc = WmiRequest::CreateWmiRequest("select * from Win32_Processor");
  if (wmiSystemReq && wmiSystemReqProc &&
      !wmiSystemReq->results().empty() &&
